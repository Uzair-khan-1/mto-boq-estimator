...
from mto_boq.mto_generator import compute_reinforcement_summary, generate_mto, get_display_quantity_unit, group_by_category, mto_to_dataframe
    ...
    with st.form("project_setup_form"):
        ...
        with c2:
            unit_system = st.selectbox(
                "Unit System",
                ["FPS", "SI"],
                index=["FPS", "SI"].index(pi.unit_system),
                format_func=lambda v: "FPS (feet/inches, sft/cft - Pakistani market practice)" if v == "FPS" else "SI (metric - m/mm, sqm/m3)",
                help="Controls the units shown for every dimension input, and for MTO/BOQ quantities. Internal calculations always run in metric regardless of this choice.",
            )
            ...
            wall_thickness_mm = st.selectbox("Default Wall Thickness (mm)", [100, 115, 150, 200, 230], index=4, help="Initial default only - refine per-project in feet/inches or metric on the verification step.")
...
def step_3():
    ...
    unit_label = "FPS (feet for lengths, inches for cross-sections, sft for areas)" if unit_system == "FPS" else "SI (metric)"
    st.markdown(
        "Review every value below and edit anything that doesn't match your actual drawing. "
        f"Displaying in **{unit_label}**. Confidence levels: {legend}",
    ...
    with st.expander("General", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            params.num_floors = render_estimate_input("Number of RCC slab levels (incl. roof)", params.num_floors, "num_floors_inp", "count", unit_system)
        with c2:
            params.plinth_area_per_floor_sqm = render_estimate_input("Plinth/built-up area per floor", params.plinth_area_per_floor_sqm, "plinth_area_inp", "area", unit_system)
    ...
    with st.expander("Footings", expanded=True):
        ...
        with c1:
            params.footings.count = render_estimate_input("Count", params.footings.count, "ftg_count", "count", unit_system)
        with c2:
            params.footings.length_m = render_estimate_input("Length", params.footings.length_m, "ftg_len", "length", unit_system)
        with c3:
            params.footings.width_m = render_estimate_input("Width", params.footings.width_m, "ftg_wid", "length", unit_system)
        with c4:
            params.footings.depth_m = render_estimate_input("Depth", params.footings.depth_m, "ftg_dep", "length", unit_system)

    with st.expander("Columns", expanded=True):
        st.caption("Width/depth (cross-section) shown in inches under FPS; height shown in feet.")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            params.columns.count = render_estimate_input("Count", params.columns.count, "col_count", "count", unit_system)
        with c2:
            params.columns.width_m = render_estimate_input("Width (b)", params.columns.width_m, "col_b", "thickness", unit_system)
        with c3:
            params.columns.depth_m = render_estimate_input("Depth (d)", params.columns.depth_m, "col_d", "thickness", unit_system)
        with c4:
            params.columns.height_per_floor_m = render_estimate_input("Height/floor", params.columns.height_per_floor_m, "col_h", "length", unit_system)

    with st.expander("Beams", expanded=True):
        st.caption("Width/depth (cross-section) shown in inches under FPS; length shown in feet.")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            params.beams.count = render_estimate_input("Count per level", params.beams.count, "beam_count", "count", unit_system)
        with c2:
            params.beams.avg_length_m = render_estimate_input("Avg length", params.beams.avg_length_m, "beam_len", "length", unit_system)
        with c3:
            params.beams.width_m = render_estimate_input("Width", params.beams.width_m, "beam_w", "thickness", unit_system)
        with c4:
            params.beams.depth_m = render_estimate_input("Depth", params.beams.depth_m, "beam_d", "thickness", unit_system)

    with st.expander("Slabs", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            params.slabs.area_per_floor_sqm = render_estimate_input("Area per floor", params.slabs.area_per_floor_sqm, "slab_area", "area", unit_system)
        with c2:
            params.slabs.thickness_m = render_estimate_input("Thickness", params.slabs.thickness_m, "slab_t", "thickness", unit_system)

    with st.expander("Walls / Masonry", expanded=True):
        st.caption("Wall thickness shown in inches under FPS; length/height shown in feet.")
        c1, c2, c3 = st.columns(3)
        with c1:
            params.walls.total_length_per_floor_m = render_estimate_input("Total wall length/floor", params.walls.total_length_per_floor_m, "wall_len", "length", unit_system)
        with c2:
            params.walls.height_m = render_estimate_input("Wall height", params.walls.height_m, "wall_h", "length", unit_system)
        with c3:
            params.walls.thickness_m = render_estimate_input("Wall thickness", params.walls.thickness_m, "wall_t", "thickness", unit_system)
    ...
    with st.expander("Openings (doors/windows)", expanded=True):
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            params.openings.door_count_per_floor = render_estimate_input("Doors/floor", params.openings.door_count_per_floor, "door_count", "count", unit_system)
        with c2:
            params.openings.avg_door_area_sqm = render_estimate_input("Avg door area", params.openings.avg_door_area_sqm, "door_area", "area", unit_system)
        with c3:
            params.openings.window_count_per_floor = render_estimate_input("Windows/floor", params.openings.window_count_per_floor, "win_count", "count", unit_system)
        with c4:
            params.openings.avg_window_area_sqm = render_estimate_input("Avg window area", params.openings.avg_window_area_sqm, "win_area", "area", unit_system)
...
def step_4():
    ...
    st.caption("This check is always evaluated in kg/m\u00b3 (metric) internally, regardless of your selected unit system.")

    df = mto_to_dataframe(mto_items, unit_system)
    st.dataframe(df, use_container_width=True, hide_index=True)

    st.markdown("##### Calculation breakdown / traceability")
    st.caption("Quantities below are shown in your selected unit system. Formula inputs are shown in metric (SI) for consistency.")
    ...
    for category, items in grouped.items():
        with st.expander(f"{category} ({len(items)} item{'s' if len(items) != 1 else ''})"):
            for item in items:
                ...
                st.write(f"Quantity: **{disp_qty:,.3f} {disp_unit}**")
                st.caption(f"Formula: {item.formula}")
                if item.inputs_used:
                    st.caption("Inputs used (metric): " + ", ".join(f"{k}={v}" for k, v in item.inputs_used.items()))
...
def step_5():
    ...
    st.caption(f"Unit system: **{pi.unit_system}** \u00b7 Currency: **{pi.currency}** (change these back in Step 1 if needed)")
